...
def train():
    ...
    for it in range(start_step, opt.endEpoch):
        ...
        if it % opt.disp_interval == 0 and it:
            ...
            log_text = 'Iter-{}; Was_D:{:.3f}; Euclidean_loss_target:{:.3f};reg_loss:{:.3f}; G_loss: {:.3f}; D_loss_real: {:.3f};' ' D_loss_fake: {:.3f}; att_loss:{:.3f}; x_loss:{:.3f}; rl: {:.2f}%; fk: {:.2f}%; '\
                        .format(it, Wasserstein_D.item(),  Euclidean_loss_target.item(),reg_loss.item(),
                                G_loss.item(), D_loss_real.item(), D_loss_fake.item(), Att_loss.item(), X_loss.item(), acc_real * 100, acc_fake * 100)
            print(log_text)
