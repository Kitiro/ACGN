...
def test_zsl(datadir, test_input, netG, opt, dataset):
    ...
    screened_sample, screened_label, trans_sample, trans_label = sample_screen_unseen(G_sample, test_sim_label, opt)
    screen_classifier = Softmax(screened_sample, screened_label)
    screen_acc = screen_classifier.acc(dataset.test_unseen_feature.numpy(), unseen_label)
    trans_classifier = Softmax(trans_sample, trans_label)
    trans_acc = trans_classifier.acc(dataset.test_unseen_feature.numpy(), unseen_label)

    print("Acc: {:.2f}%  Screen Acc: {:.2f}% Trans Acc: {:.2f}".format(acc, screen_acc, trans_acc))
...
def sample_screen_unseen(test_data, test_label, opt):
    ...
    for i in range(opt.att_dim):
        if os.path.exists(opt.classfier_root+'/{}.m'.format(i)):
            ...
            none_list.append(i)
    tt_labels = np.unique(test_label)
    screened_sample = []
    screened_label = []
    for data, claind in zip(test_data,test_label):
        maxscore = 0
        pre_label = claind
        for cla in tt_labels:
            score = 1
            for i in range(opt.att_dim):
                if i not in none_list:
                    att_value = int(np.round(opt.test_att[cla-opt.ntrain, i]*10))
                    pred = models[i].predict_proba(data.reshape(1,-1))
                    if att_value in labels_list[i]:
                        prob = pred[0][labels_list[i].index(att_value)]
                    else:
                        att_up = att_value + 1
                        att_down = att_value - 1
                        try:
                            prob = np.max((pred[0][labels_list[i].index(att_up)], pred[0][labels_list[i].index(att_down)]))
                        except:
                            prob = 0
                    score *= prob
            if maxscore <= score:
                ...
                pre_label = cla
        if pre_label == cla:
            screened_sample.append(data)
            screened_label.append(claind)
    screened_sample = np.array(screened_sample)
    screened_label = np.array(screened_label)
    
    transfered = np.empty([0,2048])
    trans_label = []
    for i in np.unique(test_label):
        temp_data = screened_sample[np.where(screened_label==i)[0]] 
        ori_data = test_data[np.where(test_label==i)[0]]
        ori_cent = np.mean(ori_data, axis=0)
        new_cent = np.mean(temp_data, axis=0)
        dis = new_cent - ori_cent
        trans_sample = temp_data - dis

        transfered = np.vstack((transfered, trans_sample))
        trans_label.extend(i for j in temp_data)


    return(screened_sample, screened_label, transfered, np.array(trans_label))
