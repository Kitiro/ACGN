...
parser.add_argument('--endEpoch', type = int, default = 3000, help= 'train epoch')
...
parser.add_argument('--cent_w', type = float, default = 5, help= 'CENT_LADMBDA')
...
def train():
    ...
    dataset_train = TensorDataset(
        torch.cat((preinputs.train_sim_label.long(), generated_unseen_label)),  # target label 
        torch.cat((preinputs.train_sim_output, mapping_semantic_unseen)), # target real visual feature 
        torch.cat((preinputs.train_sim_feature,  generated_unseen_x)),# source real visual feature
        torch.cat((preinputs.train_sim_att_2, generated_unseen_att)), # target attributes
    )

    train_loader = DataLoader(
        dataset=dataset_train, batch_size=opt.batchsize, shuffle=True, num_workers=8
    )
    ...
    for it in range(start_step, opt.endEpoch):
        ...
        #cnt1, cnt2 = 0, 0
        for cnt, (labels, feat_data, x, att) in enumerate(train_loader):
            ...
            """ Discriminator """
            if cnt % opt.D_epoch != opt.D_epoch:
                x = x.float().cuda()
                att = att.float().cuda()
                X = feat_data.float().cuda()
                y_true = labels.long().cuda()
            ...
            """ Generator """
            if cnt % opt.D_epoch == opt.D_epoch-1:
                #blobs = data_layer.forward()
                ...
                # GAN's G loss
                G_loss = opt.Adv_LAMBDA * torch.mean(D_fake)
                ...
                all_loss = -G_loss + C_loss + reg_loss + Att_loss + X_loss + Euclidean_loss_target
                all_loss.backward()
                ...
                log_text = 'Iter-{}; Was_D:{:.3f}; Euclidean_loss_target:{:.3f};reg_loss:{:.3f}; G_loss: {:.3f}; C_loss: {:.3f}; D_loss_real: {:.3f};' ' D_loss_fake: {:.3f}; att_loss:{:.3f}; x_loss:{:.3f}; rl: {:.2f}%; fk: {:.2f}%; '\
                            .format(it, Wasserstein_D.item(),  Euclidean_loss_target.item(),reg_loss.item(),
                                    G_loss.item(), C_loss.item(), D_loss_real.item(), D_loss_fake.item(), Att_loss.item(), X_loss.item(), acc_real * 100, acc_fake * 100)
                print(log_text)
