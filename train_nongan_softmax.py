...
parser.add_argument('--s_w', type = float, default = 10, help= 'weight of Semantic_loss')
...
def train():
    ...
    t_l = np.unique(data_layer.get_whole_data()['labels'])
    ...
    mapping_semantic_unseen = mapping_semantic[dataset.unseenclasses]
    ...
    for it in range(start_step, opt.endEpoch):
        ...
        for _ in range(opt.G_epoch):
            ...
            all_loss = Att_loss + X_loss + Euclidean_loss_target + reg_loss + semantic_loss
        ...
        if it % opt.disp_interval == 0 and it:
            ...
            log_text = 'Iter-{}; Euclidean_loss_target:{:.3f};reg_loss:{:.3f}; att_loss:{:.3f}; x_loss:{:.3f}; Semantic_loss:{:.3f}'.format(it, Euclidean_loss_target.item(),reg_loss.item(),Att_loss.item(), X_loss.item(), semantic_loss.item())
    ...
    with open(f'data/AwA2/Nongan/Kcla{opt.k_class}_Kinst{opt.k_inst}_results.txt', 'a') as out:
        out.write('Att_w:{}_X_w:{}_Centroid_w:{}_Semantic_w:{}\nAcc:{}_StdAcc:{}\n'.format(opt.att_w, opt.x_w, opt.cent_w, opt.s_w, result.best_acc, result.best_acc_std))
...
def eval_fakefeat_test(it, netG, preinputs, opt, result, dataset):
    ...
    # classifier = Classifier(G_sample, test_sim_label, _nclass = dataset.ntrain_class+ dataset.ntest_class)
    # acc, acc2 = classifier.acc(dataset.test_unseen_feature, unseen_label)
...
def eval_fakefeat_test_gzsl(it, netG, preinputs, opt, result, dataset):
    ...
    # S-->T
    acc_S_T, _ = classifier.acc(dataset.test_seen_feature.numpy(), dataset.test_seen_label.numpy())

    # U-->T
    acc_U_T, _ = classifier.acc(dataset.test_unseen_feature.numpy(), unseen_label)
