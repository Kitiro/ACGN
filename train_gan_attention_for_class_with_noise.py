...
from dataset_for_attention import FeatDataLayer, DATA_LOADER, KnnFeat_attention, SVM, Softmax, Classifier
...
def train():
    ...
    for it in range(start_step, opt.endEpoch):
        ...
        for _ in range(opt.D_epoch):
            ...
            bat_mean, bat_std = torch.mean(x, dim=-1, keepdim=True).repeat(1,1,opt.x_dim), torch.std(x, dim=-1, keepdim=True).repeat(1,1,opt.x_dim) 
            x = (torch.normal(bat_mean, bat_std) * opt.scale + x).cuda()
        ...
        if it % opt.evl_interval == 0 and it >= 40:
            ...
            netG.train()
    result_file = 'results/attention_result_'+opt.dataset+'_noise.txt'
    with open(result_file,'a') as out:
        out_info = ''
        out_info = 'Kcla{:d}_Kinst{:d}_Att_w:{}_x_w:{}_Cent_W:{}_Selected_ratio:{}_Split_num:{}_Scale:{}_Seed:{}_Acc:{}_Acc2:{}\n'.format(opt.k_class,opt.k_inst,opt.att_w, opt.x_w, opt.cent_w, opt.selected_ratio, opt.split_num, opt.scale, opt.manualSeed, result.best_acc, result.best_acc_std)
        if opt.gzsl:
