...
from models import _netD, _netCC_3, _param, _netD_SC
...
def train():
    ...
    with open(result_file,'a') as out:
        out_info = 'Kcla{:d}_Kinst{:d}_Att_w:{}_x_w:{}_c_w:{}_Acc{}\n'.format(opt.k_class,opt.k_inst,opt.att_w, opt.x_w, opt.cent_w best_acc)
...
def eval_fakefeat_test(it, netG, preinputs, opt, result, dataset):
    ...
    print(torch.unique(test_sim_label))
    print(dataset.ntrain_class)
    print(torch.unique(test_sim_label-dataset.ntrain_class))
...
def eval_fakefeat_test_gzsl(it, netG, preinputs, opt, result, dataset):
    ...
    # test_seen_sim_feature, test_seen_sim_label = dataset.train_feature.numpy(), dataset.train_label.numpy()
    ...
    out_file = f'data/{opt.dataset}/gan/Kcla{opt.k_class}_Kinst{opt.k_inst}_Att_w:{opt.att_w}_X_w:{opt.x_w}_Centroid_w:{opt.cent_w}_gzsl.log'
