...
class DATA_LOADER(object):
    def __init__(self, opt):
        ...
        self.tr_cls_centroid = np.zeros(
            [self.seenclasses.shape[0], self.feature_dim], np.float32
        )  # .astype(np.float32)
    ...
    def read_matdataset(self, opt):
        ...
        print(np.unique(self.test_unseen_label))
        print(np.unique(self.test_seen_label))
...
class FeatDataLayer(object):
    def __init__(self, label, feat_data, x, att1, att2, opt):
        ...
        self._epoch = 0
        cprint('Whole data length:{}, need {} iterations.'.format(len(self._x), int(len(self._x)/self._opt.batchsize)+1), 'blue')
    ...
    def _get_next_minibatch(self):
        ...
        minibatch_att1 = np.array([self._att1[i] for i in db_inds]).squeeze()
...
class Unseen_FeatDataLayer(object):
    def __init__(self, data, att, label, opt):
        """used to prepare noise+att for unseen class samples generation at training stage"""
        self._opt = opt
        self._att = att
        self._label = label
        self._data = data
    ...
    def _get_next_minibatch(self):
        ...
        minibatch_data = np.array([self._data[i] for i in db_inds])
        minibatch_att = np.array([self._att[i] for i in db_inds]).squeeze()
        minibatch_label = np.array([self._label[i] for i in db_inds])
...
class TestKnn(object):
    ...
    def _get_test_seen_data(self):
        ...
        for k in seen_source_input.keys():
            ...
            # print(k)
            # print(seen_source_input[k].size())
            # print(seen_target_label[k].size())
            # print(seen_target_att[k].size())
            ...
            if opt.random:
                ...
                for i, j in zip(
                    ...
                    train_sim_feature.append(
                        self.dataset.train_feature.numpy()[
                            np.where(self.dataset.train_label.numpy() == class_dic[j])[
                                0
                            ]
                        ][seed]
                    )
                ...
                # file = h5py.File('/data/liujinlu/zsl_glove/data/%s/KnnFeat_random.h5'%opt.dataset,'w')
                # file.create_dataset('train_sim_feature' , data = train_sim_feature.numpy())
                # file.create_dataset('train_sim_att_1' , data = train_sim_att_1.numpy())
                # file.create_dataset('train_sim_att_2' , data = train_sim_att_2.numpy())
                # file.create_dataset('train_sim_output' , data = train_sim_output.numpy())
                # file.create_dataset('train_sim_label' , data = train_sim_label.numpy())

                # file.create_dataset('test_sim_feature' , data = test_sim_feature.numpy())
                # file.create_dataset('test_sim_att_1' , data = test_sim_att_1.numpy())
                # file.create_dataset('test_sim_att_2' , data = test_sim_att_2.numpy())
                # file.create_dataset('test_sim_label' , data = test_sim_label.numpy())
                # file.create_dataset('test_sim_output' , data = test_sim_output.numpy())

                # file.create_dataset('test_seen_sim_feature' , data = test_seen_sim_feature.numpy())
                # file.create_dataset('test_seen_sim_att_1' , data = test_seen_sim_att_1.numpy())
                # file.create_dataset('test_seen_sim_att_2' , data = test_seen_sim_att_2.numpy())
                # file.create_dataset('test_seen_sim_label' , data = test_seen_sim_label.numpy())
                # file.create_dataset('test_seen_sim_output' , data = test_seen_sim_output.numpy())
                # file.close()
                ...
                if os.path.isfile(
                    os.path.join(
                        self.datapath,
                        "%s/KnnFeat_%d_%d.h5" % (opt.dataset, opt.k_class, opt.k_inst),
                    )
                ):
                    print("read data2 ...")
                    file = h5py.File(
                    ...
                    train_sim_att_1 = torch.from_numpy(
                        file["train_sim_att_1"][:].squeeze()
                    ).float()
                    ...
                    test_sim_feature = torch.from_numpy(
                        file["test_sim_feature"][:]
                    ).float()
                    test_sim_att_1 = torch.from_numpy(
                        file["test_sim_att_1"][:].squeeze()
                    ).float()
                    test_sim_att_2 = torch.from_numpy(
                        file["test_sim_att_2"][:].squeeze()
                    ).float()
                    test_sim_label = torch.from_numpy(file["test_sim_label"][:]).int()
                    test_sim_output = torch.from_numpy(
                        file["test_sim_output"][:]
                    ).float()

                    test_seen_sim_feature = torch.from_numpy(
                        file["test_seen_sim_feature"][:]
                    ).float()
                    test_seen_sim_att_1 = torch.from_numpy(
                        file["test_seen_sim_att_1"][:].squeeze()
                    ).float()
                    ...
                    test_seen_sim_output = torch.from_numpy(
                        file["test_seen_sim_output"][:]
                    ).float()
                    ...
                    class_dic = {}  # 记录每一个训练类在train_classes中的相似类的序号,通过attribute寻找相似类
                    class_knn = NearestNeighbors(
                        n_neighbors=opt.k_class + 1, metric="cosine"
                    ).fit(self.dataset.train_att)
                    ...
                    inst_knn = {}  #
                    for i in np.unique(self.dataset.train_label.numpy()):
                        sim_cla_feat = self.dataset.train_feature.numpy()[
                            np.where(self.dataset.train_label.numpy() == i)[0]
                        ]  # 类别i的样本
                        inst_knn[i] = NearestNeighbors(
                            n_neighbors=opt.k_inst, metric="cosine"
                        ).fit(
                            sim_cla_feat
                        )  # 通过第i类训练的KNN
                        # print(i, sim_cla_feat.shape)

                    # 生成 train_feature 的输入
                    train_sim_feature = []  # end-to-end input when training
                    train_sim_att_1 = []  # att of input
                    train_sim_att_2 = []  # att of output
                    train_sim_output = []  # end-to-end output when training
                    train_sim_label = []
                    cou = 0
                    seen_class_samples = {}
                    for seen_class in np.unique(self.dataset.train_label):
                        seen_class_samples[
                            seen_class
                        ] = self.dataset.train_feature.numpy()[
                            np.where(self.dataset.train_label.numpy() == seen_class)[0]
                        ]  # all samples of class 'clas'
                    ...
                    for i, j in tqdm(
                        ...
                        for cla in class_dic[
                            ...
                            cou += 1
                            d, ind = inst_knn[cla].kneighbors(
                                i.reshape(1, -1)
                            )  # 找出cla这个类中与i最近的样本，返回它们的index
                            temp_data = seen_class_samples[cla]
                            for ii in ind[0]:
                                train_sim_feature.append(
                                    temp_data[ii]
                                ...
                                train_sim_output.append(np.array(i))  # ground truth
                                train_sim_label.append(j)  # ground truth
                    ...
                    train_sim_feature = (
                        np.array(train_sim_feature).squeeze().astype(float)
                    )
                    train_sim_feature = torch.from_numpy(
                        train_sim_feature
                    )  # label 和 train_label一致
                    train_sim_att_1 = np.array(train_sim_att_1).astype(float)
                    train_sim_att_1 = torch.from_numpy(train_sim_att_1)
                    ...
                    test_seen_sim_feature = []
                    test_seen_sim_att_1 = []  # 暂时不用
                    test_seen_sim_att_2 = []
                    test_seen_sim_label = []
                    test_seen_sim_output = []  # 暂时不用
                    ...
                    for i in np.unique(self.dataset.test_seen_label.numpy()):
                        for cla in class_dic[i]:
                            ...
                            for j in range(len(temp_feat)):
                                test_seen_sim_feature.append(temp_feat[j])
                                test_seen_sim_label.append(i)
                                test_seen_sim_att_1.append(
                                    np.array(self.dataset.train_att[cla])
                                )
                                test_seen_sim_att_2.append(
                                    np.array(self.dataset.train_att[i])
                                )
                                test_seen_sim_output.append(i)
                    test_seen_sim_feature = (
                    ...
                    test_seen_sim_att_2 = (
                        np.array(test_seen_sim_att_2).squeeze().astype(float)
                    )
                    test_seen_sim_att_2 = torch.from_numpy(test_seen_sim_att_2)
                    test_seen_sim_att_1 = (
                        np.array(test_seen_sim_att_1).squeeze().astype(float)
                    )
                    test_seen_sim_att_1 = torch.from_numpy(test_seen_sim_att_1)
                    test_seen_sim_output = (
                        np.array(test_seen_sim_output).squeeze().astype(float)
                    )
                    test_seen_sim_output = torch.from_numpy(test_seen_sim_output)
                    ...
                    for i in np.unique(self.dataset.test_unseen_label.numpy()):
                        d, ind = test_class_knn.kneighbors(
                            self.dataset.test_att[i].reshape((1, -1))
                        )  # 找到当前unseen类最相似的seen类
                        for cla in ind[0]:
                            temp_feat = self.dataset.train_feature.numpy()[
                                np.where(self.dataset.train_label.numpy() == cla)[0]
                            ]
                            for j in range(len(temp_feat)):
                                test_sim_feature.append(temp_feat[j])
                                test_sim_label.append(i + self.dataset.ntrain_class)
                                test_sim_att_1.append(
                                    np.array(self.dataset.train_att[cla])
                                )
                                test_sim_att_2.append(
                                    np.array(self.dataset.test_att[i])
                                )
                                test_sim_output.append(i)
                    test_sim_feature = (
                    ...
                    test_sim_att_1 = np.array(test_sim_att_1).squeeze().astype(float)
                    test_sim_att_1 = torch.from_numpy(test_sim_att_1)
                    test_sim_att_2 = np.array(test_sim_att_2).squeeze().astype(float)
                    test_sim_att_2 = torch.from_numpy(test_sim_att_2)
                    test_sim_output = np.array(test_sim_output).squeeze().astype(float)
                    test_sim_output = torch.from_numpy(test_sim_output)
                    ...
                    file.create_dataset(
                        "test_seen_sim_att_1", data=test_seen_sim_att_1.numpy()
                    )
                    file.create_dataset(
                    ...
                    file.create_dataset(
                        "test_seen_sim_output", data=test_seen_sim_output.numpy()
                    )
...
class Classifier():
    def __init__(self, tr_x, tr_y, _nclass, _nepoch=25, _batch_size=256, _lr = 0.001, _beta1 = 0.5):
        self.nclass = _nclass
        ...
        self.loader = DataLoader(
            dataset=TensorDataset(
                torch.from_numpy(tr_x),
                torch.from_numpy(tr_y),
                ), 
            batch_size=_batch_size, shuffle=True, num_workers=2, drop_last=True
        )
        self.train()
...
class FeatDataLayer_Attention(object):
    def __init__(self, label, feat_data, x, att1, att2, opt, distance):
        """Set the roidb to be used by this layer during training."""
        # self._roidb = roidb
        assert len(label) == feat_data.shape[0]
        self._opt = opt
    ...
    def _get_next_minibatch(self):
        ...
        minibatch_att2 = np.array([self._att2[i] for i in db_inds]).squeeze()
        minibatch_label = np.array([self._label[i] for i in db_inds])
        minibatch_distance = np.array([self.distance[i] for i in minibatch_label])
