...
# 本脚本为对抗式
# 将D的分类损失修改为logsoftmax, D的分类器改为Linear Softmax。 
# 基础上添加attention机制，在输入时，对输入的多个sample. e.g., K*seen samples -> 1 unseen sample. 
# K*seen samples -> attention -> 1 mixed sample -> 1 unseen sample. 
import torch
...
from dataset_for_attention_unique_unseen import FeatDataLayer, DATA_LOADER, KnnFeat_attention, Classifier, map_label, Softmax
from models import  _param, _netD_SC, _netCC_attention_multi_head, _netA
...
def train():
    ...
    for it in pbar:
        ...
        vout, logits = netA(att, train=True)  # dim: 85, 2048
        att2v_mse_loss = mse_loss(vout, X)
        att_cls_loss = 0.5*D_criterion(logits, y_true)
        att2v_loss = att2v_mse_loss + att_cls_loss
        ...
        pbar.set_description('Att2v_loss:{:^7.3f};MSE_loss:{:^7.3f};CE_loss:{:^7.3f};Acc:{:^7.3f}%;'.format(att2v_loss.item(),att2v_mse_loss.item(),att_cls_loss.item(),acc_real*100))
    netA.eval()
    for it in range(start_step, opt.endEpoch):
        ...
        for _ in range(opt.D_epoch):
            ...
            att = netA(att)
            G_sample = netG(x, att) # generated sample, hidden state
        ...
        for _ in range(opt.G_epoch):
            ...
            y_true = Variable(torch.from_numpy(labels.astype('int'))).view(-1).cuda()
            att = netA(att)
            G_sample, G_att = netG(x, att, train=True)
            _, C_real = netD(X)
            D_fake, C_fake = netD(G_sample)
        ...
        if it % opt.evl_interval == 0 and it >= 40:
            netG.eval()
            eval_fakefeat_test(it, netG, netA, preinputs, opt, result, dataset)
            if result.save_model and opt.save_model:
                ...
                for _i in files2remove:
                    os.remove(_i)
                save_model(it, netG, netD, netA, opt.manualSeed, log_text,
                           out_subdir + '/Best_model_ZSL_Acc_{:.2f}_Epoch_{:d}.tar'.format(result.acc_list[-1],it))
            if opt.gzsl:
                eval_fakefeat_test_gzsl(it, netG, netA, preinputs, opt, result_gzsl, dataset)
...
def save_model(it, netG, netD, netA, random_seed, log, fout):
    torch.save({
        'it': it + 1,
        'state_dict_G': netG.state_dict(),
        'state_dict_D': netD.state_dict(),
        'state_dict_A': netA.state_dict(),
        'random_seed': random_seed,
        'log': log,
    }, fout)


def eval_fakefeat_test(it, netG, netA, preinputs, opt, result, dataset):
    ...
    while(True):
        if cur+opt.batchsize <= len(test_sim_feature):
            ...
            temp_att = netA(temp_att)
            g_sample = netG(temp_x.cuda(), temp_att)
...
def eval_fakefeat_test_gzsl(it, netG, netA, preinputs, opt, result, dataset):
    # test_unseen
    test_sim_feature, test_sim_att_2, test_sim_label = preinputs.test_sim_feature, preinputs.test_sim_att_2.view(-1, opt.att_dim), preinputs.test_sim_label.view(-1)
    netG.eval()
