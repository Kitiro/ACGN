...
class DATA_LOADER(object):
    ...
    def read_matdataset(self, opt):
        ...
        #     self.attribute = preprocessing.scale(self.attribute, axis=0)
        ...
        if opt.validation:
            ...
            if os.path.isfile(self.file_path):
                ...
                class_knn = NearestNeighbors(
                    n_neighbors=self.dataset.attribute.shape[0], metric="cosine"
                ).fit(mask_attribute)
                ...
                for c in self.dataset.unseenclasses.numpy():
                    temp = []
                    for tar in ind[c]:
                        if tar in invalid_c:
                            continue
                ...
                for c in invalid_c:  # 用于生成unseen的seen类。为了避免与生成的unseen类分布重合，为它们选择距离最远的seen类作为source。
                    ...
                    for tar in ind[c][1:][::-1]:
                        if tar in invalid_cc or tar in self.dataset.unseenclasses or tar == c:
                            continue
                        elif len(temp) >= opt.k_class:
                            break
                        else:
                ...
                left_seen = [i for i in self.dataset.seenclasses.numpy() if i not in invalid_c]
                for c in left_seen:
                    temp = []
                    for tar in ind[c][1:]:
                        if tar in invalid_cc or tar in self.dataset.unseenclasses or tar == c:
